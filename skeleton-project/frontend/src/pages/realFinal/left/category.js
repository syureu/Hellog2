...
class category extends Component {
  ...
  _getCategoryData = async function() {
    const getData = await axios('/realFinal/get/category');
  ...
  _addCategory = async function() {
    ...
    if (category_name) {
      ...
      if (category_name !== '' && category_name.length > 0) {
        const add = await axios('/realFinal/add/category', {
          method: 'POST',
  ...
  _removeCategory = async function(category) {
    if (window.confirm(category.name + ' 카테고리를 삭제하시겠습니까?')) {
      const remove = await axios('/realFinal/delete/category', {
        method: 'POST',
        data: category,
        headers: new Headers(),
      });
  ...
  _modifyCategory = async function(category) {
    ...
    if (modify_name !== '' && modify_name.length > 0) {
      ...
      if (
        ...
        const modfy = await axios('/realFinal/modify/category', {
          method: 'POST',
          data: data,
          headers: new Headers(),
        });
      ...
      <div className="Category">
        <ul>
          <li>
            ...
            {login && admin === 'Y' && user_ip === '192.168.0.1' ? (
              !edit ? (
                <input
                  type="button"
                  value="Edit"
