...
def main(request):
    ...
    exercise_name = machine.exercise_name
    context = {
        "username": username,
...
def manager_login(request):
    if request.method == "POST":
        ...
        res = requests.post(URL + "login", data=json.dumps(user), verify=False)
        ...
        if res.status_code == 200:
            ...
            if "USER" == getRole_res.json()[0]:
                return render(
                    request, "rasp_server/access.html", {"msg": "일반회원은 로그인 할 수 없습니다."}
                )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if res.status_code == 200:
                ...
                machine_id = Machine.objects.get(selected=True).machine_id
                machine_res = requests.get(
                    URL + "api/equipmentexercises/equipment/" + str(machine_id),
                    verify=False,
                )
                if machine_res.status_code == 200:
                    exercise_list = machine_res.json()
                    print(exercise_list)
                else:
                    exercise_list = []
                context = {
                ...
                return render(request, "rasp_server/select_exercise.html", context)
        # 로그인 실패
...
def user_nfc_login(request):
    machine_id = Machine.objects.get(selected=True).machine_id
    machine_res = requests.get(
        URL + "api/equipmentexercises/equipment/" + str(machine_id), verify=False
    )
    if machine_res.status_code == 200:
        exercise_list = machine_res.json()
        print(exercise_list)
...
def select_exercise(request, exercise_name, exercise_id):
    user = User.objects.all()[0]
    username = user.username
    role = user.role
    machine = Machine.objects.get(selected=True)
    machine_name = machine.machine_name
    machine.exercise_name = exercise_name
    machine.exercise_id = exercise_id
    machine.save()
    return render(
        request,
        "rasp_server/main.html",
        {
            "username": username,
            "role": role,
            "machine_name": machine_name,
    ...
    try:
        ...
        if nfc_res.status_code != 200:
            ...
            user_info = requests.get(
                URL + "api/users/myinfo/", headers=headers, verify=False
            ).json()
...
def renew_machines(request):
    ...
    if response.status_code == 200:
        ...
        for machine in res_content:
            print(machine["name"])
            machine_list.append({"machine_name": machine["name"]})
            new_machine = Machine(
...
def get_past_record(request, btn_name):
    ...
    if btn_name == "machine":
        ...
        response = requests.get(
            URL + "api/records/myrecord/equipment/" + equipmentId,
            headers=headers,
            verify=False,
        )
    else:
    ...
    for record in record_list:
        ...
        try:
            machine_name = Machine.objects.get(machine_id=machine_id).machine_name
        except Machine.DoesNotExist:
            continue
...
def finish_work(request):
    if request.method == "POST":
        ...
        if request.POST.get("size") == "0":
            return JsonResponse({"msg": "저장할 데이터가 없습니다.", "result": [],})
    ...
    # id 설정
    myinfo = requests.get(URL + "api/users/myinfo/", headers=headers, verify=False)
    ...
    for i in range(0, list_size):
        set_str = today_list.get("today_list[" + str(i) + "][set_cnt]").split()
        set_cnt = int(set_str[0][:1])
        weight = int(set_str[1].split("kg")[0])
        cnt = int(today_list.get("today_list[" + str(i) + "][cnt]"))
        startTime = (
            str(today_list.get("today_list[" + str(i) + "][startTime]"))[:23] + "+00:00"
        )
        endTime = (
            str(today_list.get("today_list[" + str(i) + "][endTime]"))[:23] + "+00:00"
        )
        ...
        if res_record.status_code == 200:
            ...
            msg = "서버 문제로 기기 내부에 저장됩니다. \n 서버 정상 작동시 데이터가 전송됩니다."
            result = False
            cur_record = Record(
                rid=today_record["id"],
                auth_key=auth_key,
